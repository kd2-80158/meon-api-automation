...
public final class GenerateDigilockerLink extends BaseTest {
	...
	public void verifyResponseWithMandatoryFieldsGenerateDigilockerLink(ITestContext context) {
		...
		if (digilockerUrl == null || digilockerUrl.trim().isEmpty()) {
			softAssert.assertAll(); // stops here with clear message
			return;
		}
		boolean resumed = authService.openUrlAndWaitWithTimeout(digilockerUrl, logger, 120);

		if (!resumed) {
			throw new SkipException("User did not complete Digilocker flow in time.");
		}
